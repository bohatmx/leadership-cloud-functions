...
exports.followCreated = functions.database.ref('/followGC/{followID}').onCreate((snap, context) => {
    ...
    if(companyID === "-LOs4iZh3Y9LSiNtpWlH"){
      ...
      // update CEO Details
      admin.database().ref('/user').child(ceo_id).once('value').then(function (snapshot) {
        ...
        admin.database().ref().update(updates).then(() =>{
          console.log("Following successful userID: "+userID+" CEO ID: "+honoraryUserID);
        }).catch(posts_err => {
          console.error("Following Error userID: "+userID+" CEO ID: "+honoraryUserID);
        })
      });

      // update CE Details
      admin.database().ref('/user').child(ce_id).once('value').then(function (snapshota) {
        ...
        admin.database().ref().update(ceupdates).then(() =>{
          console.log("Following successful userID: "+userID+" CE ID: "+CEUserID);
        }).catch(posts_err => {
          console.error("Following Error userID: "+userID+" CE ID: "+CEUserID);
        })
      });
    }else{
