...
module.exports = function (){
    ...
    this.thoughts = function(name, body, response){
        ...
        if((!body.companyID.trim()) || (!body.companyName.trim()) || (!body.dailyThoughtType) || (!body.dateRegistered) || (!body.dateScheduled) || (!body.journalUserID.trim()) || (!body.journalUserName.trim()) || (!body.user_type) || (!body.postingFrom)){
        ...
        if((body.dailyThoughtType < 0) || (body.dailyThoughtType > 3)){
            response.status(400).json({"code": "400", "status": "Failure","message":"Please check thought type before posting."});
            return;
        }
        ...
        var active = true, status = config.postStatus[3], companyID_status = "", dailyThoughtDescription,dailyThoughtType_status,
        topLeader_status = "false", publish_status="", stringDateRegistered,
        stringDateScheduled, dateUpdated, dailyThoughtID = 0;
        ...
        dailyThoughtDescription = config.dailyThoughtTypeString[body.dailyThoughtType];
        dailyThoughtType_status = body.dailyThoughtType +"_"+status;
        companyID_status = body.companyID+"_"+status;

        if((user_type == 4) && (status == config.postStatus[0])) topLeader_status = 'true_'+status;
        else topLeader_status = 'false';

        // Global Contributor User, Company Admin, Daily Thoughts, I-Leader, Organisation Contributor
        if ((user_type == 4) || (user_type == 7) || (user_type == 9) || (user_type == 3) || (user_type == 11) ) {
            dailyThoughtDescription = config.dailyThoughtTypeString[body.dailyThoughtType];
            dailyThoughtType_status = body.dailyThoughtType +"_"+status;
            companyID_status = body.companyID+"_"+status;
        }
        // Gold User - Master Contributor
            ...
            if(body.dailyThoughtType == 3){
                ...
                dailyThoughtType_status = body.dailyThoughtType +"_"+config.postStatus[2];
        ...
        // Corporate I-Leader
            ...
            response.status(200).json({"code": "400", "status": "Failure","message": `This account is not authorised to post, Please contact the Admin.`});
            return
        }
        ...
        else if((user_type == 7) || (user_type == 6) || (user_type == 3) || (user_type == 11) ){
            // status here can approved or unpublished
            publish_status = body.companyID+"_"+status;
        }
        // Daily thoughts user
        ...
        // Generate postID
        dailyThoughtID = admin.database().ref(dbref).push().key;
    ...
    this.articles = function(name, body, response){
        ...
        // check if all fields are provided
        if((!body.companyID.trim()) || (!body.companyName.trim()) || (!body.dailyThoughtType) || (!body.dateRegistered) || (!body.dateScheduled) || (!body.journalUserID.trim()) || (!body.journalUserName.trim()) || (!body.articleType) || (!body.user_type) || (!body.subtitle.trim()) || (!body.title.trim()) ||(!body.manualOrExternal.trim())){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }
        ...
        if((body.articleType < 0) || (body.articleType > 2) || (body.dailyThoughtType < 0) || (body.dailyThoughtType > 2)){
            response.status(400).json({"code": "400", "status": "Failure","message":"Please check article type before posting."});
            return;
        }

        var active = true, status = config.postStatus[3], companyID_status = "", dailyThoughtDescription,dailyThoughtType_status,articleTypeDescription,
        topLeader_status = "false", stringDateRegistered,
        stringDateScheduled, dateUpdated, newsID = 0;
        var user_type = body.user_type;
        ...
        // Generate postID
        newsID = admin.database().ref(dbref).push().key;
        body.newsID = newsID;
    ...
    this.podcasts = function(name, body, response){
        ...
        // check if all fields are provided
        if((!body.companyID.trim()) || (!body.companyName.trim()) || (!body.dateRegistered) || (!body.dateScheduled) || (!body.userID.trim()) || (!body.userName.trim()) || (!body.user_type) || (!body.podcastType) || (!body.title.trim()) || (!body.UploadExternal.trim()) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        if((body.podcastType < 0) || (body.podcastType > 2) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"Please check podcast type before posting."});
            return;
        }
        ...
        var active = true, status = config.postStatus[3], companyID_status = "",company_status="", dailyThoughtDescription,dailyThoughtType_status,
        topLeader_status = "false", stringDateRegistered,
        stringDateScheduled, podcastID = 0, podcastDescription = config.podcastTypeDesc[body.podcastType], storageName = body.title,podcastSize=0;
        var user_type = body.user_type;
        ...
        // Generate postID
        podcastID = admin.database().ref(dbref).push().key;
    ...
    this.videos = function(name, body, response){
        ...
        // check if all fields are provided
        if((!body.companyID.trim()) || (!body.companyName.trim()) || (!body.dateRegistered) || (!body.dateScheduled) || (!body.userID.trim()) || (!body.userName.trim()) || (!body.user_type) || (!body.title.trim()) || (!body.UploadExternal.trim()) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        if((body.UploadExternal.trim() == "external") || (body.UploadExternal.trim() == "videxternal")){
            if(!body.url.trim()){
                response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
                return;
            }
        ...
        var active = true, status = config.postStatus[3], companyID_status = "",company_status="", dailyThoughtDescription,dailyThoughtType_status,
        topLeader_status = "false", stringDateRegistered,stringDateScheduled, videoID = 0, storageName = body.title,videoSize=0,lengthInSeconds=0;
        var user_type = body.user_type;
        ...
        // Generate postID
        videoID = admin.database().ref(dbref).push().key;
    ...
    this.users = function (name, body, response) {
        var password,stringDateRegistered,dateRegistered,userDescription,companyID_userType,userType;
        ...
        // check if all fields are provided
        if((!body.companyID.trim()) || (!body.companyName.trim()) || (!body.email.trim()) || (!body.firstName.trim()) || (!body.lastName.trim()) || (!body.user_type) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        password = Math.random().toString(36).substring(2, 6) + Math.random().toString(36).substring(2, 6);
        ...
        // Generate postID
        newUploadUserID = admin.database().ref(dbref).push().key;
    ...
    this.pldptasks = function (name, body, response) {
        // check if all fields are provided
        if((!body.companyID) || (!body.companyName) || (!body.dateRegistered) || (!body.journalUserName) || (!body.moveAction) || (!body.moveActionDesc) || (!body.user_type) || (!body.title) || (!body.uid) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        if((!body.companyID.trim()) || (!body.companyName.trim()) || (!body.journalUserName.trim()) || (!body.moveAction.trim()) || (!body.moveActionDesc.trim()) || (!body.title.trim()) || (!body.uid.trim()) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }
        ...
        var active=true,stringDateRegistered,userType,myPLDPID,notificationID,reminderStatus = "Sent",stringReminderDate,userDescription;

        if((body.reminderOn === true) ){
            if((!body.reminderDate) || (!body.reminderFrequency.trim()) ){
                response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check reminder date or frequency before posting."});
                return;
            }
        ...
        // Generate postID
        myPLDPID = admin.database().ref(dbref).push().key;
        notificationID = admin.database().ref(dbref2).push().key;
    ...
    this.feedback = function (name, body, response) {
        ...
        // check if all fields are provided
        if((!body.companyID) || (!body.companyName) || (!body.category) || (!body.journalUserID) || (!body.journalUserName) || (!body.journalUserEmail) || (!body.message) || (!body.subject) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        if((!body.companyID.trim()) || (!body.companyName.trim()) || (!body.journalUserName.trim()) || (!body.journalUserID.trim()) || (!body.category.trim()) || (!body.journalUserEmail.trim()) || (!body.message.trim()) || (!body.subject.trim()) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }
        ...
        // Generate postID
        feedbackID = admin.database().ref(dbref).push().key;
    ...
    this.follower = function (name, body, response) {
        ...
        // check if all fields are provided
        if((!body.followerCompanyID) || (!body.followerCompanyName) || (!body.followerEmail) || (!body.followerFirstName) || (!body.followerLastName) || (!body.followerUserID) || (!body.parentUserID) || (!body.parentCompanyID) || (!body.parentCompanyName) || (!body.parentEmail) || (!body.parentFirstName) || (!body.parentLastName)){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        if((!body.followerCompanyID.trim()) || (!body.followerCompanyName.trim()) || (!body.followerEmail.trim()) || (!body.followerFirstName.trim()) || (!body.followerLastName.trim()) || (!body.followerUserID.trim()) || (!body.parentUserID.trim()) || (!body.parentCompanyID.trim()) || (!body.parentCompanyName.trim()) || (!body.parentEmail.trim()) || (!body.parentFirstName.trim()) || (!body.parentLastName.trim()) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        if(!body.followerPhotoURL){
            body.followerPhotoURL = "https://firebasestorage.googleapis.com/v0/b/leadershipplatform-158316.appspot.com/o/mailassets%2Fdefault-user.png?alt=media&token=ea955943-9b02-4cd9-95c0-cd1436569498";
        }

        if(!body.parentPhotoURL){
            body.parentPhotoURL = "https://firebasestorage.googleapis.com/v0/b/leadershipplatform-158316.appspot.com/o/mailassets%2Fdefault-user.png?alt=media&token=ea955943-9b02-4cd9-95c0-cd1436569498";
        }

        var dateRegistered, parentUserID, followerUserID, followerData = {}, parentData = {};
        ...
        followerData.email = body.followerEmail
        ...
        parentData.email = body.parentEmail
        ...
        // Write the new podcast's data
        var updates = {};
        updates[`/${dbref}/` + parentUserID +`/${followerUserID}`] = followerData;
        updates[`/users/` + parentUserID +`/follower/${followerUserID}`] = followerData;
        updates[`/users/` + followerUserID +`/following/${parentUserID}`] = parentData;

        admin.database().ref().update(updates).then(() => {
            response.status(200).json({"code": "200", "status": "Success","payload": {
    ...
    this.likes = function (name, body, response) {
        ...
        // check if all fields are provided
        if((!body.companyID) || (!body.companyName) || (!body.journalUserID) || (!body.journalUserName) || (!body.postType) || (!body.postID) || (!body.likeUnlike) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        if((!body.companyID.trim()) || (!body.companyName.trim()) || (!body.journalUserID.trim()) || (!body.journalUserName.trim()) || (!body.postType.trim()) || (!body.postID.trim()) || (!body.likeUnlike.trim()) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        if(!body.photoURL){
            body.photoURL = "https://firebasestorage.googleapis.com/v0/b/leadershipplatform-158316.appspot.com/o/mailassets%2Fdefault-user.png?alt=media&token=ea955943-9b02-4cd9-95c0-cd1436569498";
        }

        // set tbl ref
        var dbref = config.endpoints[body.postType.trim()]
        if((dbref == undefined) || (body.postType.trim() != "thoughts") || (body.postType.trim() != "articles") || (body.postType.trim() != "podcasts") || (body.postType.trim() != "videos")){
            response.status(400).json({"code": "400", "status": "Failure","message":"Unavailable post type! Please check post type parameter."});
            return;
        }

        var dateRegistered, data = {}, clickData = {};
        dateRegistered = Date.now();
        ...
        if((likeType === "like") || (likeType === "likes")){
            // Write the new podcast's data
            updates[dbref + '/' + data.itemID + '/likes/' + data.journalUserID] = true;
            updates[dbref + '/' + data.itemID + '/likesData/' + data.journalUserID] = data;
            updates['user-liked/' + data.userPost] = data;
            updates['user-clicks/' + data.userPost] = clickData;
        }
        else if((likeType === "unlike") || (likeType === "unlikes")){
            // remove like data
            updates[dbref + '/' + data.itemID + '/likes/' + data.journalUserID] = null;
            updates[dbref + '/' + data.itemID + '/likesData/' + data.journalUserID] = null;
        ...
        admin.database().ref().update(updates, function (error) {
            if (error) {
                ...
                if((likeType === "like") || (likeType==="likes")){
                    let countLikes = admin.database().ref(dbref).child(data.itemID).child('countLikes');
                ...
                response.status(200).json({"code": "200", "status": "Success","payload": data});
            }
        });
    ...
    this.comments = function (name, body, response) {
        ...
        // check if all fields are provided
        if((!body.companyID) || (!body.companyName) || (!body.journalUserID) || (!body.journalUserName) || (!body.postType) || (!body.postID) || (!body.comment) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }

        if((!body.companyID.trim()) || (!body.companyName.trim()) || (!body.journalUserID.trim()) || (!body.journalUserName.trim()) || (!body.postType.trim()) || (!body.postID.trim()) || (!body.comment.trim()) ){
            response.status(400).json({"code": "400", "status": "Failure","message":"There are some missing fields, please check before posting."});
            return;
        }
        ...
        // set tbl ref
        var dbref = config.endpoints[body.postType.trim()]
        console.log(body.postType.trim())
        if(dbref == undefined){
            response.status(400).json({"code": "400", "status": "Failure","message":"Unavailable post type! Please check post type parameter."});
            return;
        }

        var dateRegistered,commentID, data = {}, clickData = {};
        dateRegistered = Date.now();
        var stringDateRegistered = new Date(dateRegistered).toLocaleString();

        commentID = admin.database().ref().child('/' + dbref + '/' + body.postID.trim() + '/comments').push().key;
        ...
        // post comments
        updates['/' + dbref + '/' + body.postID.trim() + '/comments/' + commentID] = data;
